...
/// Set algebra helpers ?union / intersection / difference / symmetric
/// difference ?plus a few convenient constructors that preserve order via
/// [LinkedHashSet].
class SetUtil {
  SetUtil._();

  /// Union of [a] and [b] ?every element present in either set.
  static Set<T> union<T>(Set<T> a, Set<T> b) => {...a, ...b};

  /// Intersection of [a] and [b] ?elements present in both sets.
  static Set<T> intersection<T>(Set<T> a, Set<T> b) => a.intersection(b);

  /// [a] minus [b] ?elements in [a] that are not in [b].
